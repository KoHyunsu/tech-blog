...
React를 처음 시작했을 때 오류가 가장 많이 발생하는 지점입니다. JSX는 반드시 최상단의 단일 Tag에서 시작해야 합니다. React가 사용하는 Virtual DOM 방식에서는 컴포넌트 변화를 감지할 때 효율적으로 비교하고자 컴포넌트 내부는 하나의 DOM 트리 구조로 이루어져야 한다는 규칙이 있기 때문입니다. 이때, 최상단 단일 Tag는 \<div> 외에도 \<Fragment>, \<>를 사용할 수 있습니다.
...
&#x9;return (
&#x9;	\<div className='scg' style={style}>Hello, World!\</div>
&#x9;);
}
